...
class PremiumManifestTest {
    ...
    @Test
    fun `test premium manifest import and retrieval`() {
        runBlocking {
            // 1. Import
            val result = importer.importFromAssets("catalog/exercise-content-manifest.v2.json", force = true)
            if (result.added == 0 && result.updated == 0) {
                fail("Import failed with errors: ${result.errors.joinToString("; ")}")
            }
            assertTrue(result.added > 0 || result.updated > 0)
            ...
            assertNotNull("Risk level should exist", safety?.riskLevel)
            assertNotNull("Rep range should exist", progression?.repRange)
