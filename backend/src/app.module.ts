 ...
 *
 * As features futuras da T16 (`auth`, `ai`, `sync`, `backup`) e da T17 (`social`) entram como
 * módulos aqui dentro, no mesmo processo e no mesmo banco. Não há necessidade operacional que
...
export class AppModule implements NestModule {
  static forRoot(config: AppConfig) {
    return {
      module: AppModule,
      imports: [ConfigModule.forRoot(config), CommonModule, DatabaseModule, HealthModule],
    };
  }
