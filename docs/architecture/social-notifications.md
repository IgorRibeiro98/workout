...
| `CHALLENGE_INVITATION_RECEIVED` | `challengeInvitationReceived` | Usuário é convidado como participante de um desafio | Abre tela de Detalhe do Desafio (`Screen.ChallengeDetail`) |
...
- **`PushTokenRegistrationWorker`**: Tarefa em segundo plano com `WorkManager` para garantir envio idempotente do token ao backend com retry exponencial em caso de instabilidade de rede.
- **`onNewToken`**: Capturado em `SparkFirebaseMessagingService`, disparando o agendamento no `PushTokenRegistrationWorker`.
- **Sign Out / Troca de Conta**: O `FirebaseAuthGateway` executa um callback assíncrono que envia `DELETE /v1/social/notifications/devices/:deviceId` ao backend e limpa o `PushAccountScope` local.
...
- **`SparkFirebaseMessagingService`**:
  ...
  - Deserializa via `PushDataPayload.fromMap(remoteMessage.data)`.
  - **Account Isolation**: Compara `payload.recipientSocialId` com o perfil social ativo em memória/DataStore. Se divergente, rejeita e descarta.
  ...
  - **Canais de Notificação**:
    - `social_requests`: Importância Alta (som e vibração) para pedidos e aceites de amizade.
    - `social_challenges`: Importância Padrão para convites, avisos e término de desafios.
  - **Deep Links Seguros**: Intent explícito apontando para `MainActivity` com `EXTRA_DESTINATION` e `EXTRA_ENTITY_ID` usando flags imutáveis (`PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT`).
