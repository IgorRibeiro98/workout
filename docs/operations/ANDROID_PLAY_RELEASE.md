...
| NDK `28.2.13676358` (o padrão do AGP 9.1.1) — **opcional** | `sdkmanager "ndk;28.2.13676358"` (≈ 700 MB de download, ≈ 2,7 GB em disco) | `release.ndk.debugSymbolLevel = "SYMBOL_TABLE"` (T18.4.2) extrai a tabela de símbolos das `.so` para `BUNDLE-METADATA/com.android.tools.build.debugsymbols/` usando o `llvm-objcopy` do NDK. Sem NDK o AGP **não** baixa nada e **não** falha: pula a extração em silêncio (visível só com `--info`). Hoje as únicas `.so` do bundle vêm do AndroidX e chegam já sem `.symtab`, então o resultado é o mesmo com ou sem NDK — ver §11. |
...
| Play Console: "Este App Bundle contém código nativo, e você não fez upload dos símbolos de depuração" | O AAB traz `.so` de dependências (`libandroidx.graphics.path.so`, `libdatastore_shared_counter.so`, 4 ABIs) sem `BUNDLE-METADATA/com.android.tools.build.debugsymbols/`. Desde a T18.4.2 o release pede `SYMBOL_TABLE`, mas essas `.so` são publicadas pelo AndroidX já sem tabela de símbolos (`llvm-nm`: "no symbols"), e o AGP pula a extração ("has already been stripped") — com ou sem NDK. | Aviso informativo, não bloqueante: não há símbolo a enviar, e o app não tem código nativo próprio. Confira com `unzip -l app/build/outputs/bundle/release/app-release.aab \| grep debugsymbols`: vazio é o esperado hoje. O dia em que uma dependência trouxer uma `.so` com `.symtab`, a configuração já extrai — desde que o NDK esteja instalado (§2). Não use `FULL`: sem seção `.debug_*` nas libs ele também não produz nada. |
